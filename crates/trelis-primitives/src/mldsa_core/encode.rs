use super::module_lattice::encode::{ArraySize, Encode, EncodingSize, VectorEncodingSize};
use core::ops::Add;
use hybrid_array::{
    Array,
...
use super::algebra::{Elem, Polynomial, Vector};
...
pub(crate) mod test {
    use super::*;
    use crate::mldsa_core::module_lattice::encode::*;
    use core::ops::Rem;
        ...
        (A, B): RangeEncodingSize,
    {
        ...
        let mut rng = rand::thread_rng();
        let decoded = Polynomial::new(Array::from_fn(|_| {
            let mut x: u32 = Rng::r#gen(&mut rng);
            x %= a.0 + b.0;
            b - Elem::new(x)
        }));
